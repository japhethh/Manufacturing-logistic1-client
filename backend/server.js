...
// Send email registration endpoint
app.post("/register-supplier", async (req, res) => {
  ...
  //   email,
  //   password: crypto.randomBytes(8).toString("hex"), // Temporary password or handle separately
  //   status: "Pending",
  //   verificationToken,
  //   verificationTokenExpires,
  //   // Optionally, store gender if needed
  // });

  // Create a verification URL with the email as a query parameter
  const verificationUrl = `http://localhost:4000/verify?email=${encodeURIComponent(
    email
  )}`;
  ...
  try {
    const info = await transporter.sendMail({
      from: `"Your Company Name" <mtrcb32130@gmail.com>`, // Your sender email
      to: email,
    ...
    res
      .status(200)
      .json({ messageId: info.messageId, message: "Verification email sent." });
  } catch (error) {
...
app.get("/verify", async (req, res) => {
  ...
  if (!email) {
    return res
      .status(400)
      .json({ error: "Email is required for verification" });
  }
