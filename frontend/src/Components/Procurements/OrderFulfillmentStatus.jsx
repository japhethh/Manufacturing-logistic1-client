...
const OrderFulfillmentStatus = () => {
    ...
    setOrders(
      orders.map((order) =>
        order.id === orderId
          ? {
              ...order,
              received: order.received + quantity,
              delivered: order.received + quantity >= order.quantity,
            }
          : order
      )
    );
  ...
  const handleReturnRequest = (orderId) => {
    setOrders(
      orders.map((order) =>
        order.id === orderId ? { ...order, returnRequested: true } : order
      )
    );
  };
  ...
  const handleInvoiceMatching = (orderId) => {
    console.log(
      `Invoice matched and payment initiated for Order ID: ${orderId}`
    );
  };
      ...
      <div className="overflow-x-auto">
        ...
        <table className="hidden min-w-full bg-white shadow-md rounded-lg border border-gray-200 md:table">
          <thead className="bg-gray-800 text-white">
            <tr>
              <th className="py-3 px-4 text-left text-sm md:text-base text-white">
                Order ID
              </th>
              <th className="py-3 px-4 text-left text-sm md:text-base text-white">
                Item
