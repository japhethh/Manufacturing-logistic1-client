...
const PurchaseOrderView = () => {
    ...
    const apiURL = "http://localhost:4000/api/purchaseorder"; // Your API URL

    useEffect(() => {
        const fetchOrder = async () => {
            try {
                const response = await axios.get(`${apiURL}/${purchaseOrderId}`);
                    ...
                    <tbody>
                        {order.items.map((item, index) => (
                            <tr key={index}>
                                ...
                                <td className="border">{(item.quantity * item.pricePerUnit).toFixed(2)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>

                <h2 className="text-lg font-semibold mt-4">Total Amount: ${order.totalAmount.toFixed(2)}</h2>
                <h2 className="text-lg font-semibold mt-4">Delivery Date: {order.deliveryDate}</h2>
                <h2 className="text-lg font-semibold mt-4">Payment Terms: {order.paymentTerms}</h2>
